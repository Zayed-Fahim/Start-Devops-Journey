...
/**
 * Badges always carry their TEXT LABEL. Colour is reinforcement, never the
 * sole signal — around 1 in 12 men has some form of colour vision deficiency,
 * and a violet pill and a blue pill are indistinguishable to many of them.
 * The label also survives greyscale printing and a screenshot run through a
 * filter.
 */
...
export function StatusBadge({ status }: { status: Status }) {
  return (
    <span className={cn(base, STATUS_STYLES[status])}>
      {/* Decorative only — the word next to it is the real information, so
          hide the dot from screen readers rather than announcing "bullet". */}
      <span
        aria-hidden="true"
