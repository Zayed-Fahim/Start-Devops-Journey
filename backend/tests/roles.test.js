...
async function main() {
  ...
  check(
    ...
    jar.has('access_token') && jar.has('refresh_token') && jar.has('csrf_token'),
  );
  check(
    'login payload role is the role name, not an id',
    login.body?.user?.role === 'ADMIN',
    JSON.stringify(login.body?.user?.role),
  );

  const me = await call('GET', '/api/auth/me');
  check('/api/auth/me returns 200', me.status === 200, `got ${me.status}`);
  check(
    '/api/auth/me role is a string name',
    me.body?.role === 'ADMIN',
    JSON.stringify(me.body?.role),
  );
  check(
    '/api/auth/me carries permissions',
    Array.isArray(me.body?.permissions) && me.body.permissions.includes('roles.manage'),
  );
  ...
  check(
    'stats.roles contains the three system roles',
    ['ADMIN', 'DEVELOPER', 'USER'].every((r) => stats.body.roles.includes(r)),
    JSON.stringify(stats.body?.roles),
  );
  check('stats.byRole is keyed by role name', typeof stats.body?.byRole?.ADMIN === 'number');
  check(
    'stats.byRole counts sum to total',
    Object.values(stats.body.byRole).reduce((a, b) => a + b, 0) === stats.body.total,
    `${JSON.stringify(stats.body.byRole)} vs ${stats.body.total}`,
  );
  ...
  check(
    'every listed user has a string or null role',
    list.body.data.every((u) => typeof u.role === 'string' || u.role === null),
  );
  check(
    'no user leaks a raw enum column',
    list.body.data.every((u) => !('Role' in u)),
  );

  const filtered = await call('GET', '/api/users?role=ADMIN&limit=100');
  check(
    'filter by role name returns only that role',
    filtered.body.data.length > 0 && filtered.body.data.every((u) => u.role === 'ADMIN'),
    `${filtered.body.data.length} rows`,
  );

  const unknownFilter = await call('GET', '/api/users?role=NOT_A_ROLE');
  check(
    'filtering by an unknown role yields an empty page, not an error',
    unknownFilter.status === 200 && unknownFilter.body.data.length === 0,
    `got ${unknownFilter.status}`,
  );

  const sortedAsc = await call('GET', '/api/users?sortBy=role&order=asc&limit=100');
  const namesAsc = sortedAsc.body.data.map((u) => u.role ?? '');
  check(
    'sortBy=role asc is ordered by role name',
    sortedAsc.status === 200 && namesAsc.every((n, i) => i === 0 || namesAsc[i - 1] <= n),
    namesAsc.slice(0, 5).join(','),
  );
  ...
  check(
    'sortBy=role desc is ordered by role name',
    sortedDesc.status === 200 && namesDesc.every((n, i) => i === 0 || namesDesc[i - 1] >= n),
    namesDesc.slice(0, 5).join(','),
  );
  ...
  check(
    'creating a custom role returns 201',
    created.status === 201,
  ...
  check(
    'the custom role shows up in stats.roles',
    statsAfterCreate.body.roles.includes(roleName),
    JSON.stringify(statsAfterCreate.body.roles),
  );
  check(
    'the custom role starts with zero users',
    statsAfterCreate.body.byRole[roleName] === 0,
    String(statsAfterCreate.body.byRole[roleName]),
  );
  ...
  check(
    'creating a user with a custom role returns 201',
    newUser.status === 201,
    `got ${newUser.status} ${newUser.text.slice(0, 160)}`,
  );
  const userId = newUser.body?.data?.id ?? newUser.body?.id;
  const createdRole = newUser.body?.data?.role ?? newUser.body?.role;
  check(
    'the created user carries the custom role name',
  ...
  check(
    'filtering by the custom role finds that user',
    filteredCustom.body.data.some((u) => u.id === userId),
  );
  ...
  check(
    'reassigning to another role returns 200',
    reassign.status === 200,
    `got ${reassign.status}`,
  );
  check(
    'reassignment is reflected in the response',
    reassigned === 'DEVELOPER',
  ...
  check(
    'reassigning back to the custom role works',
    backRole === roleName,
    JSON.stringify(backRole),
  );
  ...
  check(
    'the error lists the roles that do exist',
    detail.includes('ADMIN') && detail.includes(roleName),
    detail,
  );
  ...
  check(
    'creating with an unknown role is a 400',
  ...
  check(
    'deleting a role that is still assigned is a 409',
    deleteInUse.status === 409,
    `got ${deleteInUse.status}`,
  );

  const adminRoleId = (await call('GET', '/api/roles')).body.data.find(
    (r) => r.name === 'ADMIN',
  )?.id;
  const deleteSystem = await call('DELETE', `/api/roles/${adminRoleId}`);
  check(
    'deleting a system role is refused',
    deleteSystem.status === 400,
    `got ${deleteSystem.status}`,
  );
  ...
  check(
    ...
    `got ${registered.status} ${registered.text.slice(0, 160)}`,
  );
  check(
    'a fresh registration lands on USER',
    registered.body?.user?.role === 'USER',
    JSON.stringify(registered.body?.user?.role),
  );
  ...
  check(
    're-login as admin after the register rotation',
    relogin.status === 200,
    `got ${relogin.status}`,
  );
  ...
  if (registeredId) {
    ...
    check(
      'registered probe user deleted',
      dropped.status === 204 || dropped.status === 200,
      `got ${dropped.status}`,
    );
  }
  ...
  check(
    'probe user deleted',
    droppedUser.status === 204 || droppedUser.status === 200,
    `got ${droppedUser.status}`,
  );

  const droppedRole = await call('DELETE', `/api/roles/${roleId}`);
  check(
    'the now-unused custom role can be deleted',
    droppedRole.status === 204,
    `got ${droppedRole.status}`,
  );

  const finalStats = await call('GET', '/api/users/stats');
  check(
    'stats no longer lists the deleted role',
    !finalStats.body.roles.includes(roleName),
    JSON.stringify(finalStats.body.roles),
  );
