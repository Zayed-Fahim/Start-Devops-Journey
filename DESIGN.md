...
decision _before_ implementation so that when something breaks during a rebuild, you can
...
- Password reset by email, MFA, OAuth / social login
- Kubernetes, Helm, service mesh (a later project, once this one is boring)
- Horizontal scaling, read replicas

> **Superseded 2026-08-14.** Authentication was originally a non-goal — users
> were _records_, not _accounts_. That changed: auth is now built by hand,
> deliberately, because the security mechanics are the point. Roles became real
> permissions at the same time. See §7a.
...
container to _start_, not to be _usable_. Postgres takes several seconds to initialize on
...
> **Superseded 2026-08-14.** The database is now hosted Supabase Postgres, not a
> local container, so the named-volume argument below is history rather than
> current practice. The _principle_ is unchanged and now applies to Supabase:
> take a backup before every migration, keep migrations forward-only, and never
> reach for "wipe it and start over" as a debugging step. What changed is only
> where the bytes live — and with a hosted database, "it survives `docker compose
down`" is replaced by "it survives me, which also means I can no longer fix it
> by deleting it."
...
**Decision: named volume.** Bind mounts are correct for _source code_ (that's what
`./backend:/usr/app` is for — live reload), and wrong for _database internals_.
...
Hand-built, not delegated to a library or to Supabase Auth. The mechanics are the
lesson.
...
| Method | Path                 | Purpose                                    |
| ------ | -------------------- | ------------------------------------------ |
| `POST` | `/api/auth/register` | create an account, always with role `USER` |
| `POST` | `/api/auth/login`    | exchange credentials for a session         |
| `POST` | `/api/auth/refresh`  | rotate the session                         |
| `POST` | `/api/auth/logout`   | revoke the session family                  |
...
| ------------------ | -------------------------- | ----------------------------- |
| Type               | JWT, HS256                 | 384-bit opaque random         |
| Lifetime           | 15 minutes                 | 7 days                        |
| Stored server-side | no                         | yes, as an HMAC-SHA256 digest |
| Cookie             | `httpOnly`, `SameSite=Lax` | `httpOnly`, `SameSite=Lax`    |

**Why a JWT for access and an opaque token for refresh.** The access token is
verified on every request, so it must be cheap — a signature check, no database
round trip. The refresh token is presented rarely and must be _revocable_, which a
stateless JWT cannot be. Different jobs, different instruments.

**Why the refresh token is hashed at rest.** The database stores an HMAC digest,
never the token itself. A database leak therefore yields no usable sessions. This
is the same reasoning as password hashing, applied to a credential people forget
is a credential.

**Why rotation with reuse detection.** Every refresh issues a new token and
revokes the old one, all tagged with a `familyId`. Presenting an already-revoked
token means two parties hold the same secret — a theft signal — so the entire
family is revoked and both are logged out. Without this, a stolen refresh token is
valid for its full lifetime and nothing ever notices.

### Defence in depth

| Control                                                   | Against                                          |
| --------------------------------------------------------- | ------------------------------------------------ |
| `httpOnly` cookies                                        | XSS reading the session                          |
| CSRF double-submit token, compared with `timingSafeEqual` | cross-site forged writes                         |
| bcrypt compare runs even for unknown emails               | user enumeration by response time                |
| Account lockout, 5 failures / 15 min                      | online password guessing                         |
| Rate limits: 10 logins / 15 min, 5 registrations / hour   | brute force                                      |
| Postgres RLS enabled on every table                       | the Supabase anon key bypassing the API entirely |
| helmet, CORS allowlist with credentials                   | header and origin attacks                        |

**RLS deserves emphasis.** The API can be flawless and the data still exposed:
Supabase serves PostgREST over the same database using the publishable key. With
RLS off, that key reads `users` and `refresh_tokens` directly. RLS is enabled with
_no policies_, which denies anon access outright; the backend connects as the table
owner and is exempt. "My API is secure" and "my data is secure" are different
claims.
...
| Action                 | Required               |
| ---------------------- | ---------------------- |
| read users, stats      | any authenticated user |
| create, update, delete | `ADMIN`                |

The UI hides what a non-admin cannot do, but the API is the real gate — hiding a
button is presentation, not security.
...
| `/users/[id]` _(optional, phase 4b)_ | Server Component                      | Detail view                           |
...
**Badges** — role and status must be distinguishable _without relying on color alone_
...
> **Superseded 2026-08-14.** The database moved to hosted Supabase Postgres and
> the Compose stack is being rewritten by hand, so Layer 1 below no longer
> applies as written. The live env contract is in
> [README.md](README.md#environment-variables): `backend/.env` holds
> `DB_HOST` / `DB_PORT` / `DB_NAME` / `DB_USER` / `DB_PASSWORD`, from which
> `src/lib/env.js` builds the Prisma connection strings, plus the `JWT_*`
> secrets. The two-URL frontend split (§ below) is unchanged and still the most
> important part of this section.

Two layers, and they are different things. Confusing them is why the original
`docker-compose.yaml` could not start.
...
| #   | File                                       | Defect                                                                                    |
| --- | ------------------------------------------ | ----------------------------------------------------------------------------------------- |
| 1   | _(missing)_ root `.env`                    | Absent entirely — Compose cannot interpolate any `${VAR}`                                 |
